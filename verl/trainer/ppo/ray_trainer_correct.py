    ...
    def __init__(self,
        ...
        if self.use_reference_policy:
            if config.algorithm.kl_ctrl.type == 'fixed':
                ...
                self.kl_ctrl_correction = core_algos.FixedKLController(kl_coef=config.algorithm.kl_ctrl.kl_coef_correction)
            elif config.algorithm.kl_ctrl.type == 'adaptive':
                assert config.algorithm.kl_ctrl.horizon > 0, f'horizon must be larger than 0. Got {config.critic.kl_ctrl.horizon}'
                self.kl_ctrl = core_algos.AdaptiveKLController(init_kl_coef=config.algorithm.kl_ctrl.kl_coef,
                                                               target_kl=config.algorithm.kl_ctrl.target_kl,
                                                               horizon=config.algorithm.kl_ctrl.horizon)
                self.kl_ctrl_correction = core_algos.AdaptiveKLController(init_kl_coef=config.algorithm.kl_ctrl.kl_coef_correction,
                                                               target_kl=config.algorithm.kl_ctrl.target_kl_correction,
                                                               horizon=config.algorithm.kl_ctrl.horizon)
            else:
                raise NotImplementedError
        else:
        ...
        for epoch in range(self.config.trainer.total_epochs):
            ...
            for batch_dict in self.train_dataloader:
                ...
                with _timer('step', timing_raw):
                    ...
                    pre_batch = pre_batch.repeat(repeat_times=self.config.actor_rollout_ref.rollout.n, interleave=True)
                    batch = gen_batch_output.union(pre_batch)
                    ...
                    with _timer('old_log_prob', timing_raw):
                        ...
                        ### Modify attention mask for the batch_correction
                    ...
                    if self.use_critic:
                        with _timer('values', timing_raw):
                            ...
                            values = self.critic_wg.compute_values(batch_correction)
                            batch_correction = batch_correction.union(values)
                    ...
                    with _timer('adv', timing_raw):
                        ...
                        if self.use_rm:
                            ...
                            reward_tensor = self.rm_wg.compute_rm_score(batch_correction)
                        ...
                        batch_correction = compute_advantage(batch_correction,
                                                  adv_estimator=self.config.algorithm.adv_estimator,
                                                  gamma=self.config.algorithm.gamma,
                                                  lam=self.config.algorithm.lam,
                                                  num_repeat=self.config.actor_rollout_ref.rollout.n)
                ...
                metrics.update(wrap_correction(compute_data_metrics(batch=batch, use_critic=self.use_critic)))
                metrics.update(wrap_correction(compute_timing_metrics(batch=batch, timing_raw=timing_raw)))
