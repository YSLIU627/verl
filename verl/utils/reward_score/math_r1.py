...
def extract_solution(solution_str):
    ...
    if "Assistant:" in solution_str:
        ...
        return None
    unique_counts = ["<think>", "</think>", "<answer>", "</answer>"]
    pass_unique_test = True
    for unique in unique_counts:
        if solution_str.count(unique) != 1:
            pass_unique_test = False
    if not pass_unique_test:
        return None
    pattern = r'^\s*<think>.*?</think>\s*<answer>.*?</answer>.*$'
    if not bool(re.fullmatch(pattern, solution_str, re.DOTALL)):
        return None
...
def compute_score_val(solution_str, ground_truth) -> float:
    retval = 0.
    try:
